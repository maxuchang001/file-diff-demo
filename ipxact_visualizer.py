...
class IPXACTVisualizer:
    def __init__(self, xml_file):
        ...
        self.ns = {"spirit": "http://www.spiritconsortium.org/XMLSchema/SPIRIT/1685-2014"}
    ...
    def generate_interface_view(self):
        ...
        for intf in self.root.findall(".//spirit:busInterface", self.ns):
            ...
            bus_type = bus_type_elem.get("spirit:name") if bus_type_elem is not None else "Unknown"
    ...
    def extract_ipxact_elements(self, root):
        ...
        if comp is not None:
            elements['component'].append({
                ...
                'version': comp.findtext("spirit:version", default="N/A", namespaces=ns),
                'vendor': comp.findtext("spirit:vendor", default="N/A", namespaces=ns),
                'library': comp.findtext("spirit:library", default="N/A", namespaces=ns)
            })
        ...
        for bus in root.findall(".//spirit:busDefinition", ns):
            bus_data = {
                ...
                'directConnection': bus.findtext("spirit:directConnection", default="N/A", namespaces=ns),
                'isAddressable': bus.findtext("spirit:isAddressable", default="N/A", namespaces=ns)
            }
        ...
        for space in root.findall(".//spirit:addressSpace", ns):
            ...
            for block in space.findall(".//spirit:addressBlock", ns):
                block_data = {
                    ...
                    'baseAddress': block.findtext("spirit:baseAddress", default="N/A", namespaces=ns),
                    'range': block.findtext("spirit:range", default="N/A", namespaces=ns),
                    'width': block.findtext("spirit:width", default="N/A", namespaces=ns)
                }
        ...
        for design in root.findall(".//spirit:design", ns):
            design_data = {
                ...
                'version': design.findtext("spirit:version", default="N/A", namespaces=ns),
                'componentinstances': []
            }
            ...
            for ci in design.findall(".//spirit:componentInstance", ns):
                ci_data = {
                    ...
                    'componentRef': ci.find("spirit:componentRef", ns).get("spirit:name") if ci.find("spirit:componentRef", ns) is not None else "N/A",
                    'version': ci.find("spirit:componentRef", ns).get("spirit:version") if ci.find("spirit:componentRef", ns) is not None else "N/A"
                }
        ...
        for param in root.findall(".//spirit:parameter", ns):
            elements['parameter'].append({
                ...
                'value': param.findtext("spirit:value", default="N/A", namespaces=ns),
        ...
        for intf in root.findall(".//spirit:busInterface", ns):
            elements['businterface'].append({
                ...
                'type': intf.find("spirit:busType", ns).get("spirit:name") if intf.find("spirit:busType", ns) is not None else "N/A",
                'mode': 'master' if intf.find("spirit:master", ns) is not None else 'slave'
            })
        # memoryMaps
        for mm in root.findall(".//spirit:memoryMap", ns):
            ...
            for ab in mm.findall(".//spirit:addressBlock", ns):
                ab_data = {
                    ...
                    'baseAddress': ab.findtext("spirit:baseAddress", default="N/A", namespaces=ns),
        ...
        for reg in root.findall(".//spirit:register", ns):
            reg_data = {
                ...
                'address': reg.findtext("spirit:addressOffset", default="N/A", namespaces=ns),
            ...
            for field in reg.findall(".//spirit:field", ns):
                field_data = {
                    ...
                    'bitOffset': field.findtext("spirit:bitOffset", default="N/A", namespaces=ns),
                    'bitWidth': field.findtext("spirit:bitWidth", default="N/A", namespaces=ns),
                    'access': field.findtext("spirit:access", default="N/A", namespaces=ns)
                }
        ...
        for view in root.findall(".//spirit:view", ns):
            elements['view'].append({
                ...
                'envIdentifier': view.findtext("spirit:envIdentifier", default="N/A", namespaces=ns)
            })
        # ports
        for port in root.findall(".//spirit:port", ns):
            elements['port'].append({
                'name': port.findtext("spirit:name", default="N/A", namespaces=ns),
                'direction': port.findtext("spirit:wire/spirit:direction", default="N/A", namespaces=ns)
            })
        # componentInstances
        for inst in root.findall(".//spirit:componentInstance", ns):
            elements['componentinstance'].append({
                ...
                'componentRef': inst.find("spirit:componentRef", ns).get("spirit:name") if inst.find("spirit:componentRef", ns) is not None else "N/A"
            })
        # adhocConnections
        ...
        for choice in root.findall(".//spirit:choice", ns):
            elements['choice'].append({
                'name': choice.findtext("spirit:name", default="N/A", namespaces=ns),
                'value': choice.findtext("spirit:value", default="N/A", namespaces=ns)
            })
        # vendorExtensions
    ...
    def generate_component_instances_diff_diagram(self, elements1, elements2, output_img="comp_inst_diff"):
        """生成组件实例差异图"""
        try:
            # 提取节点信息
            ...
            for name in set(nodes1.keys()) | set(nodes2.keys()):
                if name in nodes1 and name in nodes2:
                    if nodes1[name] != nodes2[name]:
                        # 修改的节点
                        dot.node(name, f"{name}\n({nodes1[name]['componentRef']} → {nodes2[name]['componentRef']})", shape='box', style='filled', fillcolor='orange')
                    else:
                        # 未修改的节点
                        dot.node(name, f"{name}\n({nodes1[name]['componentRef']})", shape='box', style='filled', fillcolor='lightgrey')
                elif name in nodes1:
                    # 删除的节点
                    dot.node(name, f"{name}\n({nodes1[name]['componentRef']})", shape='box', style='filled', fillcolor='red')
                else:
                    # 新增的节点
                    dot.node(name, f"{name}\n({nodes2[name]['componentRef']})", shape='box', style='filled', fillcolor='green')
    ...
    def generate_ad_hoc_connections_diff_diagram(self, elements1, elements2, output_img="adhoc_conn_diff"):
        """生成点对点连接差异图"""
        try:
            # 提取节点信息
                ...
                if name in nodes1 and name in nodes2:
                    if nodes1[name] != nodes2[name]:
                        ...
                        if len(refs1) >= 2 and len(refs2) >= 2:
                            ...
                            dot.node(name, label, shape='diamond', style='filled', fillcolor='orange')
                    else:
                        # 未修改的节点
                        refs = nodes1[name].get('internalPortReferences', [])
                        if len(refs) >= 2:
                            label = f"{name}\n({refs[0]['componentRef']}.{refs[0]['portRef']} ↔ {refs[1]['componentRef']}.{refs[1]['portRef']})"
                            dot.node(name, label, shape='diamond', style='filled', fillcolor='lightgrey')
                elif name in nodes1:
                    # 删除的节点
                    refs = nodes1[name].get('internalPortReferences', [])
                    if len(refs) >= 2:
                        label = f"{name}\n({refs[0]['componentRef']}.{refs[0]['portRef']} ↔ {refs[1]['componentRef']}.{refs[1]['portRef']})"
                        dot.node(name, label, shape='diamond', style='filled', fillcolor='red')
                else:
                    # 新增的节点
                    refs = nodes2[name].get('internalPortReferences', [])
                    if len(refs) >= 2:
                        label = f"{name}\n({refs[0]['componentRef']}.{refs[0]['portRef']} ↔ {refs[1]['componentRef']}.{refs[1]['portRef']})"
                        dot.node(name, label, shape='diamond', style='filled', fillcolor='green')
    ...
    def generate_memory_map_diff_diagram(self, elements1, elements2, output_img="memory_map_diff"):
        """生成内存映射差异图"""
        try:
            # 提取内存映射信息
            ...
            for name in set(mem_maps1.keys()) | set(mem_maps2.keys()):
                if name in mem_maps1 and name in mem_maps2:
                    if mem_maps1[name] != mem_maps2[name]:
                        # 修改的内存映射
                        dot.node(name, name, shape='box', style='filled', fillcolor='orange')
                    else:
                        # 未修改的内存映射
                        dot.node(name, name, shape='box', style='filled', fillcolor='lightgrey')
                ...
                if name in mem_maps1:
                    for block in mem_maps1[name].get('addressblocks', []):
                        ...
                        if block_in_v2:
                            # 检查是否有变化
                            if block['baseAddress'] != block_in_v2['baseAddress'] or block['range'] != block_in_v2['range'] or block['width'] != block_in_v2['width']:
                                # 修改的地址块
                                label = f"{block['name']}\nBase: {block['baseAddress']} → {block_in_v2['baseAddress']}\nRange: {block['range']} → {block_in_v2['range']}\nWidth: {block['width']} → {block_in_v2['width']}"
                                dot.node(block_name, label, shape='box', style='filled', fillcolor='orange')
                            else:
                                # 未修改的地址块
                                label = f"{block['name']}\nBase: {block['baseAddress']}\nRange: {block['range']}\nWidth: {block['width']}"
                                dot.node(block_name, label, shape='box', style='filled', fillcolor='lightgrey')
                        else:
                            # 删除的地址块
                            label = f"{block['name']}\nBase: {block['baseAddress']}\nRange: {block['range']}\nWidth: {block['width']}"
                            dot.node(block_name, label, shape='box', style='filled', fillcolor='red')
                ...
                if name in mem_maps2:
                    for block in mem_maps2[name].get('addressblocks', []):
                        ...
                        if not block_in_v1:
                            # 新增的地址块
                            label = f"{block['name']}\nBase: {block['baseAddress']}\nRange: {block['range']}\nWidth: {block['width']}"
                            dot.node(block_name, label, shape='box', style='filled', fillcolor='green')
                            dot.edge(name, block_name)
    ...
    def generate_ipxact_diff_html(self, file1, file2, output_file):
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                future_to_task = {
                    # 组件相关
                    executor.submit(visualizer1.generate_component_diagram): "component1",
                    executor.submit(visualizer2.generate_component_diagram): "component2",
                    executor.submit(self.generate_component_diff_diagram, elements1, elements2): "component_diff",
                    executor.submit(visualizer1.generate_component_instances_diagram): "componentinstance1",
                    executor.submit(visualizer2.generate_component_instances_diagram): "componentinstance2",
                    executor.submit(self.generate_component_instances_diff_diagram, elements1, elements2): "componentinstance_diff",
                    
                    # 总线相关
                    executor.submit(visualizer1.generate_bus_definition_diagram): "busdefinition1",
                    executor.submit(visualizer2.generate_bus_definition_diagram): "busdefinition2",
                    executor.submit(self.generate_bus_definition_diff_diagram, elements1, elements2): "busdefinition_diff",
                    executor.submit(visualizer1.generate_bus_interfaces_diagram): "businterface1",
                    executor.submit(visualizer2.generate_bus_interfaces_diagram): "businterface2",
                    executor.submit(self.generate_bus_interfaces_diff_diagram, elements1, elements2): "businterface_diff",
                    
                    # 设计相关
                    executor.submit(visualizer1.generate_design_diagram): "design1",
                    executor.submit(visualizer2.generate_design_diagram): "design2",
                    executor.submit(self.generate_design_diff_diagram, elements1, elements2): "design_diff",
                    
                    # 视图相关
                    executor.submit(visualizer1.generate_view_diagram): "view1",
                    executor.submit(visualizer2.generate_view_diagram): "view2",
                    executor.submit(self.generate_view_diff_diagram, elements1, elements2): "view_diff",
                    
                    # 地址空间相关
                    executor.submit(visualizer1.generate_address_space_diagram): "addressspace1",
                    executor.submit(visualizer2.generate_address_space_diagram): "addressspace2",
                    executor.submit(self.generate_address_space_diff_diagram, elements1, elements2): "addressspace_diff",
                    ...
                    executor.submit(self.generate_register_diff_diagram, elements1, elements2): "register_diff",
                    
                    # 内存映射相关
                    executor.submit(visualizer1.generate_memory_map_diagram): "memorymap1",
                    executor.submit(visualizer2.generate_memory_map_diagram): "memorymap2",
                    executor.submit(self.generate_memory_map_diff_diagram, elements1, elements2): "memorymap_diff"
                }
            ...
            # 为每个部分生成内容
            sections = [
                'component', 'componentinstance', 'busdefinition', 'businterface',
                'design', 'view', 'addressspace', 'memorymap', 'register'
            ]
            for section in sections:
                ...
                # 添加对应的图片
                if section == 'component' and results.get('component1') and results.get('component2'):
                ...
                elif section == 'componentinstance' and results.get('componentinstance1') and results.get('componentinstance2'):
                    html_content += '<div class="diagram-section">'
                ...
                elif section == 'busdefinition' and results.get('busdefinition1') and results.get('busdefinition2'):
                    html_content += '<div class="diagram-section">'
                ...
                elif section == 'businterface' and results.get('businterface1') and results.get('businterface2'):
                    html_content += '<div class="diagram-section">'
                    ...
                    if results.get('businterface_diff'):
                        html_content += '<div class="section-title">Bus Interface Differences</div>'
                        html_content += f'<div class="image-container"><img src="{results["businterface_diff"]}"></div>'
                    html_content += '</div>'

                elif section == 'design' and results.get('design1') and results.get('design2'):
                    html_content += '<div class="diagram-section">'
                    ...
                    if results.get('design_diff'):
                        html_content += '<div class="section-title">Design Differences</div>'
                        html_content += f'<div class="image-container"><img src="{results["design_diff"]}"></div>'
                    html_content += '</div>'

                elif section == 'view' and results.get('view1') and results.get('view2'):
                    html_content += '<div class="diagram-section">'
                    ...
                    if results.get('view_diff'):
                        html_content += '<div class="section-title">View Differences</div>'
                        html_content += f'<div class="image-container"><img src="{results["view_diff"]}"></div>'
                    html_content += '</div>'

                elif section == 'addressspace' and results.get('addressspace1') and results.get('addressspace2'):
                    html_content += '<div class="diagram-section">'
                    ...
                    if results.get('addressspace_diff'):
                        html_content += '<div class="section-title">Address Space Differences</div>'
                        html_content += f'<div class="image-container"><img src="{results["addressspace_diff"]}"></div>'
                    html_content += '</div>'

                elif section == 'register' and results.get('register1') and results.get('register2'):
                    html_content += '<div class="diagram-section">'
                    ...
                    if results.get('register_diff'):
                        html_content += '<div class="section-title">Register Differences</div>'
                        html_content += f'<div class="image-container"><img src="{results["register_diff"]}"></div>'
                    html_content += '</div>'

                elif section == 'memorymap' and results.get('memorymap1') and results.get('memorymap2'):
                    html_content += '<div class="diagram-section">'
                    ...
                    if results.get('memorymap_diff'):
                        html_content += '<div class="section-title">Memory Map Differences</div>'
    ...
    def generate_component_instances_diagram(self, output_img="component_instances"):
        ...
        try:
            ...
            for ci in self.root.findall(".//spirit:componentInstance", self.ns):
                ...
                type_name = type_elem.get("{http://www.spiritconsortium.org/XMLSchema/SPIRIT/1685-2014}name") if type_elem is not None else "N/A"
            ...
            for conn in self.root.findall(".//spirit:adHocConnection", self.ns):
                ...
                if len(refs) >= 2:
                    comp1 = refs[0].get("{http://www.spiritconsortium.org/XMLSchema/SPIRIT/1685-2014}componentRef")
                    comp2 = refs[1].get("{http://www.spiritconsortium.org/XMLSchema/SPIRIT/1685-2014}componentRef")
                    if comp1 and comp2:
    ...
    def generate_bus_interfaces_diagram(self, output_img="bus_interfaces"):
        ...
        try:
            ...
            for intf in self.root.findall(".//spirit:busInterface", self.ns):
                ...
                bus_type_name = bus_type.get("spirit:name") if bus_type is not None else "N/A"
                mode = "master" if intf.find("spirit:master", self.ns) is not None else "slave"
            ...
            for name, bus_type, mode in interfaces:
                ...
                dot.node(name, f"{name}\n({bus_type})\n{mode}", shape='box', style='filled', fillcolor=color)
    ...
    def generate_ad_hoc_connections_diagram(self, elements=None):
        ...
        for element in elements.get('adhocconnection', []):
            ...
            if len(comp_refs) == 2:
                connections.append((name, comp_refs[0], comp_refs[1], port_refs[0], port_refs[1]))
        ...
        try:
            ...
            for mm in self.root.findall(".//spirit:memoryMap", self.ns):
                ...
                for ab in mm.findall(".//spirit:addressBlock", self.ns):
                    ab_data = {
                        'name': ab.findtext("spirit:name", default="N/A", namespaces=self.ns),
                        'baseAddress': ab.findtext("spirit:baseAddress", default="N/A", namespaces=self.ns),
                        'range': ab.findtext("spirit:range", default="N/A", namespaces=self.ns),
                        'width': ab.findtext("spirit:width", default="N/A", namespaces=self.ns)
                    }
                    mm_data['addressblocks'].append(ab_data)
            ...
            for mm in memory_maps:
                mm_name = mm['name']
                dot.node(mm_name, mm_name, shape='box', style='filled', fillcolor='lightgrey')
                
                # 添加地址块节点
                for ab in mm['addressblocks']:
                    ab_name = f"{mm_name}_{ab['name']}"
                    label = f"{ab['name']}\nBase: {ab['baseAddress']}\nRange: {ab['range']}\nWidth: {ab['width']}"
                    dot.node(ab_name, label, shape='box', style='filled', fillcolor='lightblue')
    ...
    def generate_component_diagram(self, output_img="component"):
        ...
        try:
            ...
            version = component.findtext("spirit:version", default="N/A", namespaces=self.ns)
            vendor = component.findtext("spirit:vendor", default="N/A", namespaces=self.ns)

            # 提取总线接口
            bus_interfaces = []
            for bus_if in component.findall(".//spirit:busInterface", self.ns):
                bus_name = bus_if.findtext("spirit:name", default="N/A", namespaces=self.ns)
                bus_type = bus_if.findtext(".//spirit:name", default="N/A", namespaces=self.ns)
                bus_interfaces.append((bus_name, bus_type))

            # 提取端口
            ports = []
            for port in component.findall(".//spirit:port", self.ns):
                port_name = port.findtext("spirit:name", default="N/A", namespaces=self.ns)
                port_type = port.findtext(".//spirit:wire/spirit:direction", default="N/A", namespaces=self.ns)
            ...
            dot.node(name, component_label, shape='box', style='filled', fillcolor='lightblue')

            # 添加总线接口节点
            for bus_name, bus_type in bus_interfaces:
                bus_label = f"{bus_name}\n({bus_type})"
                dot.node(bus_name, bus_label, shape='box', style='filled', fillcolor='lightyellow')
                dot.edge(name, bus_name)

            # 添加端口节点
            for port_name, port_type in ports:
                port_label = f"{port_name}\n({port_type})"
                dot.node(port_name, port_label, shape='box', style='filled', fillcolor='lightgreen')
    ...
    def generate_bus_definition_diagram(self, output_img="bus_definition"):
        ...
        try:
            ...
            for bus in self.root.findall(".//spirit:busDefinition", self.ns):
                ...
                version = bus.findtext("spirit:version", default="N/A", namespaces=self.ns)
            ...
            for name, version in bus_defs:
                dot.node(name, f"{name}\nVersion: {version}", shape='box', style='filled', fillcolor='lightgreen')
    ...
    def generate_design_diagram(self, output_img="design"):
        ...
        try:
            ...
            # 获取设计名称和版本
            design_name = design.findtext("spirit:name", default="N/A", namespaces=self.ns)
            design_version = design.findtext("spirit:version", default="N/A", namespaces=self.ns)
            ...
            for ci in design.findall(".//spirit:componentInstance", self.ns):
                name = ci.findtext("spirit:name", default="N/A", namespaces=self.ns)
                type_elem = ci.find("spirit:componentRef", self.ns)
                type_name = type_elem.get("spirit:name") if type_elem is not None else "N/A"
            ...
            for conn in design.findall(".//spirit:interconnection", self.ns):
                active = conn.findtext("spirit:activeInterface", default="N/A", namespaces=self.ns)
                hier = conn.findtext("spirit:hierInterface", default="N/A", namespaces=self.ns)
            ...
            # 添加设计节点
            dot.node(design_name, f"{design_name}\nVersion: {design_version}", 
                    shape='box', style='filled', fillcolor='lightblue')
    ...
    def generate_view_diagram(self, output_img="view"):
        ...
        try:
            ...
            for view in self.root.findall(".//spirit:view", self.ns):
                ...
                env = view.findtext("spirit:envIdentifier", default="N/A", namespaces=self.ns)
            ...
            for name, env in views:
                dot.node(name, f"{name}\n({env})", shape='box', style='filled', fillcolor='lightyellow')
    ...
    def generate_register_diagram(self, output_img="register"):
        ...
        try:
            ...
            for reg in self.root.findall(".//spirit:register", self.ns):
                ...
                offset = reg.findtext("spirit:addressOffset", default="N/A", namespaces=self.ns)
                size = reg.findtext("spirit:size", default="N/A", namespaces=self.ns)
                access = reg.findtext("spirit:access", default="N/A", namespaces=self.ns)
                
                # 提取字段信息
                fields = []
                for field in reg.findall(".//spirit:field", self.ns):
                    field_name = field.findtext("spirit:name", default="N/A", namespaces=self.ns)
                    bit_offset = field.findtext("spirit:bitOffset", default="N/A", namespaces=self.ns)
                    bit_width = field.findtext("spirit:bitWidth", default="N/A", namespaces=self.ns)
                    field_access = field.findtext("spirit:access", default="N/A", namespaces=self.ns)
            ...
            for name, offset, size, access, fields in registers:
                # 创建寄存器节点
                reg_label = f"{name}\nOffset: {offset}\nSize: {size}\nAccess: {access}"
                dot.node(name, reg_label, shape='box', style='filled', fillcolor='lightblue')
                ...
                for field_name, bit_offset, bit_width, field_access in fields:
                    ...
                    dot.node(field_id, field_label, shape='box', style='filled', fillcolor='lightgreen')
    ...
    def generate_design_diff_diagram(self, elements1, elements2, output_img="design_diff"):
        ...
        try:
            ...
            # 提取组件实例
            instances1 = {inst['name']: inst for inst in design1.get('componentinstances', [])}
            instances2 = {inst['name']: inst for inst in design2.get('componentinstances', [])}
            ...
            if design1 and design2:
                if design1 != design2:
                    ...
                    dot.node('design', label, shape='box', style='filled', fillcolor='orange')
                else:
                    # 未修改的节点
                    label = f"{design1.get('name', 'N/A')}\nVersion: {design1.get('version', 'N/A')}"
                    dot.node('design', label, shape='box', style='filled', fillcolor='lightgrey')
            elif design1:
                # 删除的节点
                label = f"{design1.get('name', 'N/A')}\nVersion: {design1.get('version', 'N/A')}"
                ...
                dot.node('design', label, shape='box', style='filled', fillcolor='green')
    ...
    def generate_address_space_diff_diagram(self, elements1, elements2, output_img="address_space_diff"):
        """生成地址空间差异图"""
        try:
            # 提取地址空间信息
            ...
            for name in set(spaces1.keys()) | set(spaces2.keys()):
                if name in spaces1 and name in spaces2:
                    if spaces1[name] != spaces2[name]:
                        ...
                        dot.node(name, label, shape='box', style='filled', fillcolor='orange')
                    else:
                        # 未修改的节点
                        label = f"{name}\nRange: {spaces1[name].get('range', 'N/A')}\nWidth: {spaces1[name].get('width', 'N/A')}"
                        dot.node(name, label, shape='box', style='filled', fillcolor='lightgrey')
                    ...
                    dot.node(name, label, shape='box', style='filled', fillcolor='green')
                ...
                if name in spaces1:
                    for block in spaces1[name].get('addressblocks', []):
                        ...
                        if block_in_v2:
                            # 检查是否有变化
                            if block['baseAddress'] != block_in_v2['baseAddress'] or block['range'] != block_in_v2['range']:
                                # 修改的地址块
                                label = f"{block['name']}\nBase: {block['baseAddress']} → {block_in_v2['baseAddress']}\nRange: {block['range']} → {block_in_v2['range']}"
                                dot.node(block_name, label, shape='box', style='filled', fillcolor='orange')
                            else:
    ...
    def generate_register_diff_diagram(self, elements1, elements2, output_img="register_diff"):
        """生成寄存器差异图"""
        try:
            # 提取寄存器信息
                    ...
                    for field in registers1[name].get('fields', []):
                        ...
                        if field_in_v2:
                            ...
                            if field != field_in_v2:
                                ...
                                dot.node(field_name, label, shape='box', style='filled', fillcolor='orange')
                            else:
                                # 未修改的字段
                                label = f"{field['name']}\nBits: {field.get('bitOffset', 'N/A')}\nWidth: {field.get('bitWidth', 'N/A')}\nAccess: {field.get('access', 'N/A')}"
                                dot.node(field_name, label, shape='box', style='filled', fillcolor='lightgrey')
                        else:
                            # 删除的字段
                            label = f"{field['name']}\nBits: {field.get('bitOffset', 'N/A')}\nWidth: {field.get('bitWidth', 'N/A')}\nAccess: {field.get('access', 'N/A')}"
                            dot.node(field_name, label, shape='box', style='filled', fillcolor='red')
                        # 添加从寄存器到字段的连接
                ...
                if name in registers2:
                    for field in registers2[name].get('fields', []):
                        ...
                        if not field_in_v1:
                            # 新增的字段
                            label = f"{field['name']}\nBits: {field.get('bitOffset', 'N/A')}\nWidth: {field.get('bitWidth', 'N/A')}\nAccess: {field.get('access', 'N/A')}"
                            dot.node(field_name, label, shape='box', style='filled', fillcolor='green')
                            # 添加从寄存器到字段的连接
                            dot.edge(name, field_name)
    ...
    def generate_component_diff_diagram(self, elements1, elements2, output_img="component_diff"):
        """生成组件差异图"""
        try:
            # 提取组件信息
            comp1 = elements1.get('component', [{}])[0]
            ...
            if comp2:
                ...
                dot.node('comp2', label2, shape='box', style='filled', fillcolor='lightgreen')
    ...
    def process_file_pairs(self, file_pairs):
        """处理文件对"""
        for v1_file, v2_file in file_pairs:
            try:
                print(f"Processing XML file: {v1_file}")
                # 生成输出文件名
                base_name = os.path.splitext(os.path.basename(v1_file))[0]
                output_file = f"diff_{base_name}.html"
                
                # 解析XML文件
                tree1 = ET.parse(v1_file)
                tree2 = ET.parse(v2_file)
                root1 = tree1.getroot()
                root2 = tree2.getroot()
                
                # 生成差异报告
                html_content = self.generate_ipxact_diff_html(root1, root2, output_file)
                
                # 保存HTML文件
                with open(output_file, 'w', encoding='utf-8') as f:
                    f.write(html_content)
                
                print(f"Generated diff report: {output_file}")
                
            except Exception as e:
                print(f"Error processing diff {v1_file} vs {v2_file}: {str(e)}")
                continue
    ...
    def generate_bus_definition_diff_diagram(self, elements1, elements2, output_img="bus_definition_diff"):
        """生成总线定义差异图"""
        try:
            # 提取节点信息
            ...
            for name in set(nodes1.keys()) | set(nodes2.keys()):
                if name in nodes1 and name in nodes2:
                    if nodes1[name] != nodes2[name]:
                        # 修改的节点
                        dot.node(name, f"{name}\n({nodes1[name]['version']} → {nodes2[name]['version']})", shape='box', style='filled', fillcolor='orange')
                    else:
                        # 未修改的节点
                        dot.node(name, f"{name}\n({nodes1[name]['version']})", shape='box', style='filled', fillcolor='lightgrey')
                elif name in nodes1:
                    # 删除的节点
                    dot.node(name, f"{name}\n({nodes1[name]['version']})", shape='box', style='filled', fillcolor='red')
                else:
                    # 新增的节点
                    dot.node(name, f"{name}\n({nodes2[name]['version']})", shape='box', style='filled', fillcolor='green')
    ...
    def generate_component_instance_differences(self, xml1_path, xml2_path, output_path):
        """生成组件实例差异图"""
        try:
            # 解析两个XML文件
            ...
            # 创建有向图
            dot = graphviz.Digraph(comment="Component Instance Differences", engine='dot')
            dot.attr(rankdir='TB', dpi='300')
            ...
            for instance in instances1:
                ...
                if instance_id in [i.get('instanceName', '') for i in instances2]:
                    # 共同存在的组件
                    dot.node(instance_id, f"{instance_id}\n({component_type})", 
                            shape='box', style='filled', fillcolor='lightblue')
                else:
                    # 只在第一个文件中存在的组件
                    dot.node(instance_id, f"{instance_id}\n({component_type})", 
            ...
            for instance in instances1:
                instance_id = instance.get('instanceName', '')
                for port in instance.findall('.//{http://www.accellera.org/XMLSchema/IPXACT/1685-2014}port'):
                    port_name = port.get('name', '')
                    for connection in instance.findall('.//{http://www.accellera.org/XMLSchema/IPXACT/1685-2014}connection'):
                        if connection.get('name', '').startswith(port_name):
                            target = connection.get('target', '')
                            if target in [i.get('instanceName', '') for i in instances1 + instances2]:
...
def process_all_xml_files(xml_dir="test_files"):
    # 获取所有XML文件
    ...
    for xml_file in xml_files:
        try:
            visualizer = IPXACTVisualizer(xml_file)
            base_name = os.path.splitext(os.path.basename(xml_file))[0]
            output_file = f"diff_{base_name}.html"
            html_file = visualizer.generate_ipxact_diff_html(xml_file, xml_file, output_file)
            print(f"Successfully processed {xml_file} -> {html_file}")
        except Exception as e:
            print(f"Error processing {xml_file}: {e}")

def process_file_pairs(xml_dir="test_files"):
    """处理文件对并生成差异报告"""
    # 定义文件对
    file_pairs = [("ipxact_test_v1.xml", "ipxact_test_v2.xml")]

    print(f"Processing {len(file_pairs)} file pairs in {xml_dir}")

    # 处理每对文件
    for file1, file2 in file_pairs:
        try:
            file1_path = os.path.join(xml_dir, file1)
            file2_path = os.path.join(xml_dir, file2)
            ...
            output_file = f"diff_{base_name}.html"
            html_file = visualizer.generate_ipxact_diff_html(file1_path, file2_path, output_file)
            print(f"Successfully processed diff {file1} vs {file2} -> {html_file}")
        except Exception as e:
            print(f"Error processing diff {file1} vs {file2}: {e}")
